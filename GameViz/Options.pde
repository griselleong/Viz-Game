  ...
  Options(String _name, int _x, int _y, String default_selected) {
    ...
    for (int i=0; i<optNames.length;i++) {
      ...
      if ( i < 4)
        ...
        c = 1;
      if (c == 0) {
        options[i] = new Button(optNames[i], x + 5 + (i%5)*50, int(y + width*.01 + c*30), w, 20, -1, missionNum,-1); 
        println("c is 0. i : " + i );
      }
      else {
        println("i: " + i + " optNames[i] : " + optNames[i] + " (i%(optNames.length-i))*30, is: " + (i%(optNames.length-i))*30);
        options[i] = new Button(optNames[i], x  + 5 + ((i-4) % (optNames.length-4))*30, int(y + width*.01 + c*30), w, 20,-1, missionNum,-1);
      }
    }
  } 
